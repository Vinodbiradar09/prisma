...
const getOnlySpecifiedDetails = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    if (!id) {
      return res.status(400).json({
...
const getUserWhosAgeGTE25 = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
const inops = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const users = await prisma.user.findMany({
      where: {
        AND: [
...
const connectCategoryToBlog = async(req : Request , res : Response , next : NextFunction)=>{
    try {
...
const getBlogDetails = async(req :Request , res : Response , next : NextFunction)=>{
    try {
        const {id} = req.params;
        if(!id){
            return res.status(400).json({
